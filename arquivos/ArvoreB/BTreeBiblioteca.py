...
def _InserirElementos(Ap, ApNome, ordem, dataframe, chave):
  ...
  for i in range(tam_lin):
      ...
      reg_nome = Registro()
      reg_nome.Chave = dataframe.iloc[i, 1]
      reg_nome.Elemento = i
      ApNome = _Insere(reg_nome, ApNome, ordem)
      chave += 1
  return Ap, ApNome, chave

#Define os registros a serem inseridos
def Inserir(Ap, ApNome, chave, quant):
  ordem = quant
  ...
  if arq.lower().endswith(".csv"):
    dataframe = pd.read_csv(arq, header=None, dtype=colunas_dtype)      
  elif arq.lower().endswith((".xls", ".xlsx")):
    dataframe = pd.read_excel(arq, header=None, dtype=colunas_dtype)
  else:
    print ("Arquivo incompatível.")
    return Ap, ApNome, chave, None # Retorna None se o arquivo for incompatível
  
  #imprimindo dataframe criado do arquivo
  print("\nDataframe")
  print(dataframe)
  a = input("Digite um carater para continuar")
  Ap, ApNome, chave = _InserirElementos(Ap, ApNome, ordem, dataframe, chave)
  return Ap, ApNome, chave, dataframe
